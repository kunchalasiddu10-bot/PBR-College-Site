...
export const StudentTimetable: React.FC = () => {
  ...
  const [activeDay, setActiveDay] = useState<string>(() => {
    ...
    return today === 'Sunday' ? 'Monday' : today; // Default to Monday if sunday
  });
  ...
  useEffect(() => {
    const fetchTimetable = async () => {
      try {
        ...
        setError(err.response?.data?.message || 'Failed to fetch timetable scheduler.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="h-[60vh] flex flex-col justify-center items-center gap-3">
        ...
        <p className="text-sm font-semibold text-slate-400">Loading class schedule...</p>
      </div>
    );
  }
  ...
  // Filter classes matching selected active tab day
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mt-1">
          Review weekly slots, laboratory rooms, and faculty schedules.
        </p>
      </div>
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-full text-xs font-bold transition-all shrink-0 ${
              activeDay === day
                ...
                : 'bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
            }`}
          >
            {day}
          </button>
        ...
        {filteredSlots.length === 0 ? (
          <div className="col-span-full p-12 text-center rounded-3xl glass-card space-y-4">
            ...
            <div className="space-y-1">
              <p className="text-sm font-extrabold text-slate-800 dark:text-white">No Classes Scheduled</p>
              <p className="text-xs text-slate-400">You are free from lectures on {activeDay}!</p>
            </div>
          </div>
        ) : (
          filteredSlots.map((slot) => (
            <div key={slot._id} className="p-6 rounded-3xl glass-card hover:gradient-border border border-transparent transition-all space-y-4 relative overflow-hidden">
